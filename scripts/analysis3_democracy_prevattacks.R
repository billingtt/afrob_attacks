...
lmer_ester_prevattacks <- function(data, dv){
  
  myform <- formula(
    paste0(dv,"~", "log1p(attacks) + factor(DM_fem) + factor(DM_urban) + DM_age + DM_edu + WF_nofood + factor(supragroup) + factor(round) + Lights + Population + log1p(total_attacks_24mos) + (1|NAME_1) + (1|country)")
  )
...
base_out <-
  ...
  mutate(model = map(outcome, ~lmer_ester_prevattacks(data = nested, dv = .x)))
...
ggsave(pgrid1, filename = here::here("figures", "democracy_panels_coefs_prevattacks.png"),
